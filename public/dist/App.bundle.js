...
function makeMap(mapDiv) {
  if (!mapDiv) return;
  var map = new google.maps.Map(mapDiv, mapOtions);
  loadPlaces(map);
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    axios__WEBPACK_IMPORTED_MODULE_0___default().get("/api/search?q=".concat(this.value)).then(function (_ref) {
      var data = _ref.data;

      if (data.length) {
        searchResults.innerHTML = dompurify__WEBPACK_IMPORTED_MODULE_1___default().sanitize(searchResultsHTML(data));
        return;
      }

      searchResults.innerHTML = dompurify__WEBPACK_IMPORTED_MODULE_1___default().sanitiaze("<div class=\"search__result\">There is no <strong>".concat(_this.value, "</strong> found!</div>"));
    }).catch(function (err) {
      console.error(err);
    });
  });
  searchInput.on('keyup', function (event) {
    ...
    var current = searchResults.querySelector(".".concat(activeClass));
    var next;
...
var hearts = (0,_modules_bling__WEBPACK_IMPORTED_MODULE_1__.$$)('form.heart');
hearts.on('submit', _modules_heart__WEBPACK_IMPORTED_MODULE_5__["default"]);
}();
/******/ })()
;
